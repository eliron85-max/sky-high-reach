...
export default function StickyCoverTransition({
  first, second,
  coverScreens = 2, postScreens = 1,
  driftMaxPx = 160, secondRadiusPx = 28,
}: Props) {
  ...
  useEffect(() => {
    const apply = () => {
      ...
      if (Math.abs(c - lastCRef.current) < 0.001 && Math.abs(p - lastPRef.current) < 0.001 && Math.abs(driftInner - lastDriftRef.current) < 0.25) return;
      lastCRef.current = c; lastPRef.current = p; lastDriftRef.current = driftInner;
      const heroY = -20 * c;
      const heroScale = 1 - 0.06 * c;
      const heroDim = 0.18 * c;
      if (heroRef.current) heroRef.current.style.transform = `translate3d(0, ${heroY}px, 0) scale(${heroScale})`;
      if (dimRef.current) dimRef.current.style.background = `rgba(0,0,0,${heroDim})`;
      const secondY = (1 - c) * 100;
      if (coverRef.current) coverRef.current.style.transform = `translate3d(0, ${secondY}%, 0)`;
      if (innerRef.current) innerRef.current.style.transform = `translate3d(${driftInner}px, 0, 0)`;
    };
  ...
  return (
    <div ref={wrapRef} className="relative" style={{ height: `${(coverScreens + postScreens + 1) * 100}vh` }}>
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <div ref={coverRef} className="absolute inset-0 will-change-transform transform-gpu" style={{ transform: "translate3d(0, 100%, 0)" }}>
          <div className="h-full w-full bg-background overflow-hidden contain-paint" style={{ borderTopLeftRadius: secondRadiusPx, borderTopRightRadius: secondRadiusPx }}>
            <div ref={innerRef} className="h-full w-full will-change-transform transform-gpu">{second}</div>
          </div>
        </div>
      </div>
    </div>
