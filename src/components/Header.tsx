...
export default function Header() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0 pointer-events-none"} ${isVisible ? "lg:translate-y-0" : "lg:-translate-y-full"} lg:transition-all`}
      dir="rtl"
    >
                  ...
                  <div className="min-w-[260px] rounded-xl border border-[#c9a84c]/30 bg-gradient-to-b from-[#1a1a1a] to-[#0a0a0a] shadow-xl shadow-[#c9a84c]/10 p-4">
                    {serviceLinks.map((item) => (
                      <Link
                        key={item.path}
                        to={item.path}
                        className={`flex items-center gap-2 py-2 ${goldText} hover:brightness-125 transition`}
                      >
                        {item.label}
                      </Link>
                    ))}
      ...
      {mobileOpen && (
        <>
          ...
          {/* Drawer Panel */}
          <div
            className="fixed inset-y-0 right-0 z-[60] lg:hidden w-[85%] max-w-[360px]
          ...
          >
            ...
            <nav className="px-6 py-6 space-y-1 overflow-y-auto max-h-[calc(100vh-180px)]">
              ...
              >
                ...
                <span
                  className={`text-[17px] font-semibold ${goldText} group-hover:translate-x-[-4px] transition-transform duration-300`}
                >
                  עמוד ראשי
                </span>
              </Link>
              ...
              <div className="animate-fade-in" style={{ animationDelay: "150ms" }}>
                ...
                <div
                  className={`overflow-hidden transition-all duration-300 ease-out ${servicesOpen ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}
                >
