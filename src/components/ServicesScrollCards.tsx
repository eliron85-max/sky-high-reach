...
export default function ServicesScrollCards({ title, subtitle, items, className }: Props) {
  ...
  useEffect(() => {
    ...
    const applyPose = (el: HTMLElement, pose: CardPose, scale = 1) => {
      el.style.zIndex = String(pose.z);
      el.style.transform = `translate(-50%, -50%) translate3d(${pose.x}px, ${pose.y}px, 0px) rotate(${pose.r}deg) scale(${scale})`;
    };
